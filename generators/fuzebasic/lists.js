...
/**
 * @fileoverview Generating fuzebasic for list blocks.
 * @author daarond@gmail.com (Daaron Dwyer)
 */
'use strict';

goog.provide('Blockly.fuzebasic.lists');

goog.require('Blockly.fuzebasic');


Blockly.fuzebasic['lists_create_empty'] = function(block) {
  // Create an empty list.
  return ['array()', Blockly.fuzebasic.ORDER_ATOMIC];
};

Blockly.fuzebasic['lists_create_with'] = function(block) {
  // Create a list with any number of elements of any type.
  var code = new Array(block.itemCount_);
  for (var n = 0; n < block.itemCount_; n++) {
    code[n] = Blockly.fuzebasic.valueToCode(block, 'ADD' + n,
        Blockly.fuzebasic.ORDER_COMMA) || 'null';
  }
  code = 'array(' + code.join(', ') + ')';
  return [code, Blockly.fuzebasic.ORDER_ATOMIC];
};

Blockly.fuzebasic['lists_repeat'] = function(block) {
  ...
  var functionName = Blockly.fuzebasic.provideFunction_(
      ...
      [ 'function ' + Blockly.fuzebasic.FUNCTION_NAME_PLACEHOLDER_ +
        ...
        '  return $array;',
        '}']);
  var argument0 = Blockly.fuzebasic.valueToCode(block, 'ITEM',
      Blockly.fuzebasic.ORDER_COMMA) || 'null';
  var argument1 = Blockly.fuzebasic.valueToCode(block, 'NUM',
      Blockly.fuzebasic.ORDER_COMMA) || '0';
  var code = functionName + '(' + argument0 + ', ' + argument1 + ')';
  return [code, Blockly.fuzebasic.ORDER_FUNCTION_CALL];
};

Blockly.fuzebasic['lists_length'] = function(block) {
  // List length.
  var argument0 = Blockly.fuzebasic.valueToCode(block, 'VALUE',
      Blockly.fuzebasic.ORDER_FUNCTION_CALL) || 'array()';
  return ['count(' + argument0 + ')', Blockly.fuzebasic.ORDER_FUNCTION_CALL];
};

Blockly.fuzebasic['lists_isEmpty'] = function(block) {
  // Is the list empty?
  var argument0 = Blockly.fuzebasic.valueToCode(block, 'VALUE',
      Blockly.fuzebasic.ORDER_FUNCTION_CALL) || 'array()';
...
Blockly.fuzebasic['lists_indexOf'] = function(block) {
  ...
  var argument1 = Blockly.fuzebasic.valueToCode(block, 'VALUE',
      Blockly.fuzebasic.ORDER_MEMBER) || '[]';
  var functionName;
  if (block.getFieldValue('END') == 'FIRST'){
    // indexOf
    functionName = Blockly.fuzebasic.provideFunction_(
        'indexOf',
        [ 'function ' + Blockly.fuzebasic.FUNCTION_NAME_PLACEHOLDER_ +
        '($haystack, $needle) {',
...
Blockly.fuzebasic['lists_getIndex'] = function(block) {
  // Get element at index.
  var mode = block.getFieldValue('MODE') || 'GET';
  var where = block.getFieldValue('WHERE') || 'FROM_START';
  var at = Blockly.fuzebasic.valueToCode(block, 'AT',
      Blockly.fuzebasic.ORDER_UNARY_NEGATION) || '1';
  var list = Blockly.fuzebasic.valueToCode(block, 'VALUE',
      Blockly.fuzebasic.ORDER_FUNCTION_CALL) || 'array()';
...
Blockly.fuzebasic['lists_setIndex'] = function(block) {
  // Set element at index.
  // Note: Until February 2013 this block did not have MODE or WHERE inputs.
  var list = Blockly.fuzebasic.valueToCode(block, 'LIST',
      Blockly.fuzebasic.ORDER_MEMBER) || 'array()';
  var mode = block.getFieldValue('MODE') || 'GET';
  var where = block.getFieldValue('WHERE') || 'FROM_START';
  var at = Blockly.fuzebasic.valueToCode(block, 'AT',
      Blockly.fuzebasic.ORDER_NONE) || '1';
  var value = Blockly.fuzebasic.valueToCode(block, 'TO',
      Blockly.fuzebasic.ORDER_ASSIGNMENT) || 'null';
  ...
  function cacheList() {
    ...
    var listVar = Blockly.fuzebasic.variableDB_.getDistinctName(
  ...
  if (where == 'FIRST') {
    ...
    var code = cacheList();
    var xVar = Blockly.fuzebasic.variableDB_.getDistinctName(
...
Blockly.fuzebasic['lists_getSublist'] = function(block) {
  // Get sublist.
  var list = Blockly.fuzebasic.valueToCode(block, 'LIST',
      Blockly.fuzebasic.ORDER_MEMBER) || 'array()';
...
Blockly.fuzebasic['lists_split'] = function(block) {
  // Block for splitting text into a list, or joining a list into text.
  var value_input = Blockly.fuzebasic.valueToCode(block, 'INPUT',
      Blockly.fuzebasic.ORDER_MEMBER);
  var value_delim = Blockly.fuzebasic.valueToCode(block, 'DELIM',
      Blockly.fuzebasic.ORDER_NONE) || '\'\'';
